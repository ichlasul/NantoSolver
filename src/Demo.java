...
public class Demo {
  ...
  public static void main(final String[] args) {
    final Population<MyVector> population = Demo.createInitialPopulation(5);

    final Fitness<MyVector, Double> fitness = new MyVectorFitness();

    final GeneticAlgorithm<MyVector, Double> ga = new GeneticAlgorithm<MyVector, Double>(
        population, fitness);
   ...
   */
  private static Population<MyVector> createInitialPopulation(
      final int populationSize) {
    final Population<MyVector> population = new Population<MyVector>();
    final MyVector base = new MyVector();
    for (int i = 0; i < populationSize; i++) {
      // each member of initial population
      // is mutated clone of base chromosome
      final MyVector chr = base.mutate();
   ...
   */
  private static void addListener(final GeneticAlgorithm<MyVector, Double> ga) {
    // just for pretty print
    System.out
        .println(String.format("%s\t%s\t%s", "iter", "fit", "chromosome"));
    ...
    ga.addIterationListener(new IterartionListener<MyVector, Double>() {
      ...
      @Override
      public void update(final GeneticAlgorithm<MyVector, Double> ga) {

        final MyVector best = ga.getBest();
        final double bestFit = ga.fitness(best);
        final int iteration = ga.getIteration();

        // Listener prints best achieved solution
        System.out.println(String
            .format("%s\t%s\t%s", iteration, bestFit, best));
  ...
  public static class MyVector implements Chromosome<MyVector>, Cloneable {
    ...
    public MyVector mutate() {
      final MyVector result = this.clone();

      // just select random element of vector
      // and increase or decrease it on small value
      final int index = MyVector.random.nextInt(this.vector.length);
      final int mutationValue = MyVector.random.nextInt(3)
          - MyVector.random.nextInt(3);
     ...
     */
    @Override
    public List<MyVector> crossover(final MyVector other) {
      final MyVector thisClone = this.clone();
      final MyVector otherClone = other.clone();

      // one point crossover
      final int index = MyVector.random.nextInt(this.vector.length - 1);
      for (int i = index; i < this.vector.length; i++) {
        final int tmp = thisClone.vector[i];
    ...
    protected MyVector clone() {
      final MyVector clone = new MyVector();
  ...
  public static class MyVectorFitness implements Fitness<MyVector, Double> {
    ...
    @Override
    public Double calculate(final MyVector chromosome) {
      double delta = 0;
      final int[] v = chromosome.getVector();
      for (int i = 0; i < 5; i++) {
