...
public class Population<C extends Chromosome<C>> implements Iterable<C> {
  ...
  private List<C> chromosomes = new ArrayList<C>(
      Population.DEFAULT_NUMBER_OF_CHROMOSOMES);

  private final Random random = new Random();

  public void addChromosome(final C chromosome) {
  ...
  public C getRandomChromosome() {
    final int numOfChromosomes = this.chromosomes.size();
    // TODO improve random generator
    // maybe use pattern strategy ?
    final int indx = this.random.nextInt(numOfChromosomes);
    return this.chromosomes.get(indx);
  }

  public C getChromosomeByIndex(final int indx) {
    return this.chromosomes.get(indx);
  }

  public void sortPopulationByFitness(final Comparator<C> chromosomesComparator) {
